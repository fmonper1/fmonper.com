...
const DialogsContext = React.createContext<
    [DialogContextState, React.Dispatch<ReducerAction>] | undefined
    >(undefined);
...
const DialogProvider: React.FC<{initialValue?: DialogContextState}> = (props) => {
    const [state, dispatch] = React.useReducer(dialogReducer, props.initialValue ?? {
        index: -1,
        dialog: []
    });
    ...
    return (
        <DialogsContext.Provider
            value={value as [DialogContextState, React.Dispatch<ReducerAction>]}
        >
            {props.children}
        </DialogsContext.Provider>
    );
